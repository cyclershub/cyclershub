	...
	<div
		...
		<div class="flex flex-row gap-2 items-center">
			<div class="relative">
				...
				<span class="text-stone-600 absolute top-[-10px] left-[-10px] rounded-full p-1.5 text-[12px]">{$notifications.length}</span>
				<div class="absolute top-full right-0 min-w-[300px] px-4 py-2 w-auto bg-stone-100 border border-stone-200 rounded-lg z-50" class:hidden={!showNotifications}>
					{#if $notifications.length > 0}
						{#each $notifications as notification (notification.id)}
							<div
								class="px-4 py-2 flex flex-row items-center gap-2 justify-between notification">
								<p class="text-stone-600 mb-0 text-lg">
						...
						{/each}
					{:else}
						<span class="text-stone-600">No new notifications...</span>
					{/if}
				</div>
			</div>
